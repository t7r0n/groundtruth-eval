...
COMPANY = 'Kaelio'
REPO = 'groundtruth-eval'
PROJECT_TERMS = ['semantic', 'layer', 'agents', 'product', 'question', 'agent', 'answers', 'correct']
PROJECT_METRICS = ['semantic_coverage', 'layer_risk', 'agents_precision', 'product_latency']
PROJECT_FAILURES = ['semantic_drift', 'layer_gap', 'agents_misroute', 'product_blindspot']
PROJECT_ARCHETYPES = [{'name': 'semantic evidence replay', 'trigger': 'semantic signal changes while layer context is stale', 'expected': 'block release until cited evidence is regenerated'}, {'name': 'layer boundary probe', 'trigger': 'layer handoff crosses a policy or trust boundary', 'expected': 'route to reviewer with evidence packet'}, {'name': 'agents regression harness', 'trigger': 'agents behavior regresses against the last accepted fixture', 'expected': 'open a regression issue with trace and benchmark delta'}, {'name': 'product operator packet', 'trigger': 'product output needs a human-readable audit packet', 'expected': 'accept only if decision claims cite fixture evidence'}]
PROJECT_DIRECTION = 'An open benchmark + harness that scores any context layer grounded analytics agent on natural language -> governed metric correctness, designed to make Kaelio the reference standard.'
...
HERO_TITLE = 'Kaelio Adversarial Replay Board'
